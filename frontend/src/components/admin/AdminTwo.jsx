...
const AdminTwo = () => {
    ...
    const handleDownloadII = () => {
        const link = document.createElement('a');
        link.href = `/my-report.pdf`;
        link.download = `my-report.pdf`;
        link.click();
      };
      const handleDownload = () => {
        const link = document.createElement('a');
        link.href = `/report_2025.csv`;
        link.download = `report_2025.csv`;
        link.click();
      };
                  ...
                  {currentItems.map((item) => (
                    <tr key={item.email} className="hover:bg-gray-50">
                      ...
                      <td className="border border-gray-300 px-4 py-2">
                        <button className="bg-blue-500 text-white px-4 py-1 rounded-lg shadow-md hover:bg-blue-600 transition-all duration-300" onClick={handleDownloadII}>View</button>
                        </td>
                    </tr>
                  ))}
                </tbody>
